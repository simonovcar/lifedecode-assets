...
@media (max-width:860px){.ld_help_grid{grid-template-columns:1fr;} #ld_btn_menu span{display:none;} }
...
async function saveState() {
  ...
  updated_at: new Date().toISOString()
    ...
    function syncNameUI() {
      ...
      const label = isAccount ? (STATE.profile.name || "Account") : "Login";
...
if (e.target.closest("#ld_checkin_ai_ok, #ld_trigger_ai_ok")) {
  ...
  if (STATE.auth.mode === "account" && typeof openAccountInfo === "function") {
    openAccountInfo();
  } else {
    openAuthBox(true);
  }
