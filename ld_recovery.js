...
async function getSupabaseClient() {
  if (__ldSupabasePromise) return __ldSupabasePromise;

  __ldSupabasePromise = (async () => {
    const mod = await import("https://esm.sh/@supabase/supabase-js@2");
    const client = mod.createClient(CFG.SUPABASE_URL, CFG.SUPABASE_KEY, {
      auth: {
        persistSession: true,
...
async function loadState() {
  ...
  if (STATE.auth.mode === "account") {
    try {
      ...
      if (!data) {
        HAS_SERVER = true;
        await supabase.from("ld_user_state").upsert({
          user_id: user.id,
          state: STATE,
      ...
      STATE = mergeState({
        ...
        profile: {
          ...
          ...profileKeep,
          id: user.id,
          email: user.email || profileKeep.email || ""
        },
        premium: {
      ...
      console.warn("[LD] state load failed:", err.message);
      HAS_SERVER = false;
      return;
    }
  }
...
function renderAuthPanel(tab) {
  ...
  if (tab === "signup") {
    panel.innerHTML = `
      ...
      <div class="ld_hr"></div>
      <div class="ld_row">
        <button class="ld_btn" id="ld_google_login">Continue with Google</button>
        <button class="ld_btn ghost" id="ld_apple_login">Continue with Apple</button>
      </div>
    `;
    return;
  }
...
if (e.target.closest("#ld_apple_login")) {
  try {
    hideAuthError();
    setAuthLoading(true, "Redirecting to Apple...");
    await authOAuth("apple");
  } catch (err) {
    showAuthError(err.message || "Apple login failed.");
    setAuthLoading(false);
  }
  return;
}
