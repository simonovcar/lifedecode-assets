(function () {
  ...
  const CFG = {
    VERSION: "world-landing-polished-v4-auth-xp-wallpaper-fix",
    ICONIC_URL: "https://cdn.jsdelivr.net/gh/simonovcar/lifedecode-assets@main/iconic.png?v=world-iconic-wallpaper-v4",
    ICONIC_FALLBACKS: [
      "https://raw.githubusercontent.com/simonovcar/lifedecode-assets/main/iconic.png?v=world-iconic-wallpaper-v4",
      "/assets/iconic.png?v=world-iconic-wallpaper-v4",
      "/iconic.png?v=world-iconic-wallpaper-v4"
    ],
      ...
      <article class="ldw_card">
        ...
        <div class="ldw_card_body">
          <div class="ldw_checkin_box">
            ...
            <div class="ldw_note">Logged-in users get +${CFG.DAILY_XP} XP for the World when the database endpoint/table exists. If not logged in, it saves locally and queues the check-in until login.</div>
          </div>
        </div>
      </article>
    </section>
...
`;
  ...
  async function awardDailyCheckinXP(text) {
    ...
    // Best case: add this RPC in Supabase and XP becomes atomic/clean.
    const rpcAttempts = [
    ...
    for (const [fn, args] of rpcAttempts) {
      try {
        ...
        if (!error) {
          ...
          try { localStorage.setItem(CFG.PENDING_XP_KEY, JSON.stringify(payload)); } catch (e) {}
          return { ok: true, message: `Daily Check-In saved. +${CFG.DAILY_XP} XP sent to your World profile.` };
        }
      } catch (e) {}
    }

    // Fallback: write a daily_checkins row. If your DB has a trigger, it can award XP from here.
    const checkinRows = [
    ...
    return {
      ok: false,
      queued: true,
      message: `Check-In saved, but XP could not be written yet. Add RPC "ld_award_daily_checkin_xp" or a "daily_checkins" table/trigger.`
    };
  }
  ...
  function bind() {
    ...
    document.getElementById("ldw_email_login")?.addEventListener("click", async () => {
      ...
      status.textContent = error ? error.message : "Logged in. You can enter the World now. Daily XP can now be claimed.";
      updateAuthUI();
    });
    ...
    document.getElementById("ldw_google_login")?.addEventListener("click", async () => {
      ...
      const { error } = await client.auth.signInWithOAuth({ provider: "google", options: { redirectTo: CFG.WORLD_URL } });
