    ...
    <div class="ld_panel" id="ld_left">
      ...
      <div class="bd">
        <div class="ld_nav" id="ld_nav">
          <button data-open="dashboard" class="active">🏠 Dashboard <small>Overview & quick actions</small></button>
          <button data-open="checkin">✅ Daily Check-In <small>Streak + XP + mood</small></button>
          <button data-open="urge">⏱ Urge Emergency <small>90-second rescue</small></button>
          <button data-open="trigger">🧠 Trigger Decoder <small>Decode the pattern</small></button>
          <button data-open="coach">✨ Coach Output <small>12-hour plan</small></button>
          <div class="sep"></div>
          <button data-open="journal">📓 Journal <small>Quick entries</small></button>
          <button data-open="insights">📈 Insights <small>Trends & notes</small></button>
          <button data-open="tools">🧰 Tools <small>Utilities</small></button>
          <button data-open="settings">⚙️ Settings <small>Account + data</small></button>
          <button data-open="progress">🏆 Progress <small>Levels & milestones</small></button>
        </div>
      </div>
    </div>
        ...
        <div class="ld_card">
          <div class="top">
            <div>
              ...
              <p id="ld_coach_meta">Press “Generate Plan” to get your next 12-hour plan.</p>
            </div>
        ...
        <div class="ld_card">
          <div class="top">
            <div>
              ...
              <p>Interrupt the loop: breathe + name it + micro-action.</p>
            </div>
        ...
        <div class="ld_card">
          <div class="top">
            <div>
              ...
              <p>What happened, what you felt, what you did — and what you’ll do next.</p>
            </div>
...
`;
  ...
  function initLifeDecode() {
    ...
    function applyUserToState(user) {
      ...
      STATE.profile.name = (user.name || user.username || STATE.profile.name || "Member").trim();
      STATE.profile.email = user.email || "";
    }

    async function authRegister(name, email, password) {
      const data = await api(CFG.AUTH.REGISTER, {
        method: "POST",
        body: { name, email, password }
      });

      if (data && data.token) STATE.auth.token = data.token;
      if (data && data.user) applyUserToState(data.user);
      return data;
    }

    async function authLogin(email, password) {
      const data = await api(CFG.AUTH.LOGIN, {
        method: "POST",
        body: { email, password }
      });
    ...
    async function authMe() {
      const data = await api(CFG.AUTH.ME, { method: "GET" });
      if (data && data.user) {
    ...
    async function authLogout() {
      try {
        if (STATE.auth.token) {
          await api(CFG.AUTH.LOGOUT, { method: "POST" });
        }
      } catch (e) {}
    ...
    async function bootstrapAuth() {
      ...
      try {
        ...
        console.warn("[LD] auth bootstrap failed:", err.message);
        STATE.auth = clone(DEFAULT_STATE.auth);
        STATE.profile = clone(DEFAULT_STATE.profile);
        persistAuthCache();
      }
    ...
    async function loadState() {
      ...
      if (STATE.auth.mode === "account" && STATE.auth.token) {
        try {
          const data = await api(CFG.STATE.GET, { method: "GET" });
          if (data && data.state) {
            HAS_SERVER = true;
            const authKeep = clone(STATE.auth);
            const profileKeep = clone(STATE.profile);
            STATE = mergeState({ ...data.state, auth: authKeep, profile: { ...data.state.profile, ...profileKeep } });
            return;
          }
          throw new Error("No state payload.");
        } catch (err) {
          console.warn("[LD] state load failed, account still alive but state missing:", err.message);
          HAS_SERVER = false;
          return;
        }
      }
    ...
    async function saveState() {
      ...
      if (STATE.auth.mode === "account" && STATE.auth.token) {
        await api(CFG.STATE.SAVE, { method: "POST", body: { state: STATE } });
        return;
      }
    }
