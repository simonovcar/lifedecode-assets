...
function ldGetXpFromStateObject(stateObj) {
  const raw = stateObj && typeof stateObj === "object" ? stateObj.xp : 0;
  return ldXpNumber(raw);
}
...
async function ldHardSaveSharedXpToDB(reason = "xp-hard-save") {
  ...
  const mergedState = {
    ...
    ...getStateForDB(),
    xp: finalXp,
    xpMeta: ldBuildXpMeta(finalXp),
    xpLocks: { ...((remoteState && remoteState.xpLocks) || {}), ...(STATE.xpLocks || {}) },
    world: (remoteState && remoteState.world) || STATE.world || undefined,
