...
#ld_top{
  ...
  background:linear-gradient(180deg, rgba(10,12,28,.62), rgba(8,10,22,.48));
...
#ld_logo{
  ...
  overflow:hidden;
  background:
    linear-gradient(135deg, rgba(34,211,238,.95), rgba(138,91,255,.92) 55%, rgba(255,61,214,.88));
  box-shadow:
    0 10px 30px rgba(90,70,255,.28),
...
.ld_coach_output{
  ...
  border-radius:22px;
  background:
    radial-gradient(120% 180% at 0% 0%, rgba(34,211,238,.08), transparent 35%),
    radial-gradient(120% 180% at 100% 0%, rgba(138,91,255,.12), transparent 38%),
    radial-gradient(100% 140% at 50% 100%, rgba(255,61,214,.08), transparent 42%),
    linear-gradient(180deg, rgba(255,255,255,.06), rgba(255,255,255,.03));
  box-shadow:
...
<div id="ld_shell">
  <div id="ld_top">
    <div id="ld_brand">
      ...
      <span class="ld_brandname">LifeDecode Lab</span>
    </div>
...
<button data-open="urge">🚨 Emergency Button <small>Premium • Calm-down, breathing & reset guide</small></button>
        ...
        <div class="ld_card">
          <div class="top">
            <div>
             ...
             <p>Log your state, choose one simple step for now, then one follow-up step so you do not drift.</p>
            </div>
          ...
          <div class="mid">
            ...
            <div class="ld_hint" id="ld_checkin_hint">Pick your mood, then write 1 small action you will do now and 1 action after that.</div>
          </div>
    ...
    `
      <div class="ld_grid2">
        <div>
          ...
          <div class="ld_hint">
            Pick your current state, then write:
            <br>• <strong>Action 1 = what you will do in the next 5–15 minutes</strong>
            <br>• <strong>Action 2 = what you will do right after that</strong>
            <br>• <strong>Note = what is hitting you today</strong> (urge, panic, anger, frustration, stress, chaos, numbness, overthinking...)
          </div>
          ...
          <input class="ld_field" id="ld_ci_action1" placeholder="First step right now (example: drink water, shower, go outside for 5 min, close apps, eat, text friend)">
          <input class="ld_field" id="ld_ci_action2" placeholder="Second step after that (example: 10-min walk, clean desk, stretch, journal, make tea, continue work calmly)">
          <textarea class="ld_field" id="ld_ci_note" placeholder="What is the main problem today? Example: anxious after waking up, strong urges tonight, frustrated and angry, mentally overloaded, numb and unmotivated"></textarea>
        ...
        <div>
          <div class="ld_panel" style="border-radius:22px;">
            ...
            <div class="bd">
              <div class="ld_out">Write actions that are:
• small
• clear
• doable today
• specific enough that there is no confusion

GOOD examples:
• Drink water and leave the room
• Eat something and go for a 10-minute walk
• Close TikTok and shower
• Message one person and stop isolating
• Wash face and sit away from the trigger
• Open laptop and do 10 minutes only
...
Rule:
If the action feels vague, it’s weak.
If it feels obvious and easy to start, it’s good.</div>
...
async function generateCheckinComment({ mood, action1, action2, note }) {
...
- Must include:
  1. short praise or acknowledgment
  2. one smart observation about the mood / pattern
  3. one practical next suggestion
...
function buildCheckinFallbackComment(mood, action1, action2) {
  const moodMap = {
...
Now keep it simple: do "${action1 || "one small clean action"}" first.
${action2 ? `Then follow it with "${action2}".` : "Then stack one more small clean move right after it."}`;
}
...
function showCheckinAiComment(text) {
  ...
  const premiumModules = ["urge", "trigger", "journal", "insights", "progress", "tools"];
...
if (e.target.closest("#ld_checkin_ai_ok")) {
  closeSheet();
  return;
}
  ...
  try {
    ...
    if (!action1 && !note) {
  alert("Write Action 1 or explain what is hitting you today.");
  return;
}
