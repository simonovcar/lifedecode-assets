(function () {
  ...
  const CFG = {
    VERSION: "world-landing-fixed-v1",
    ICONIC_URL: "https://cdn.jsdelivr.net/gh/simonovcar/lifedecode-assets@main/iconic.png?v=world-iconic-fixed-1",
    WORLD_URL: "/world",
    POLICY_URL: "/policy",
    TERMS_URL: "/terms",
...
.ldw_hero:after{content:"YEE-HAW • HMPH • ARRR • REALLY?";position:absolute;left:50%;top:22px;transform:translateX(-50%);width:min(760px,92%);padding:10px 14px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(4,2,12,.42);color:rgba(255,255,255,.78);font-size:11px;font-weight:1000;letter-spacing:.20em;text-transform:uppercase;box-shadow:0 18px 45px rgba(0,0,0,.30);backdrop-filter:blur(10px);z-index:2;}
.ldw_hero_inner{position:relative;z-index:1;width:min(920px,100%);}
.ldw_badge{display:inline-flex;align-items:center;justify-content:center;margin-bottom:14px;padding:10px 14px;border-radius:999px;border:1px solid rgba(255,255,255,.18);background:linear-gradient(135deg,rgba(74,33,145,.62),rgba(255,68,222,.18));color:#fff;font-size:12px;font-weight:1000;letter-spacing:.16em;text-transform:uppercase;box-shadow:0 0 28px rgba(205,72,255,.20);}
...
@media(max-width:560px){.ldw_shell{width:min(100% - 16px,1360px);padding-top:8px}.ldw_top{border-radius:20px;padding:12px}.ldw_nav{width:100%;display:grid;grid-template-columns:1fr 1fr 1fr}.ldw_btn,.ldw_linkbtn{font-size:10px;padding:9px 8px;min-height:42px}.ldw_hero{min-height:610px;border-radius:24px;padding:70px 12px 22px}.ldw_hero:before{background-size:auto 63%;background-repeat:no-repeat;background-position:center 58px}.ldw_hero:after{top:15px;font-size:8px;letter-spacing:.11em;padding:8px 10px}.ldw_badge{font-size:9px;letter-spacing:.10em;padding:8px 10px}.ldw_title{font-size:clamp(35px,12vw,56px)}.ldw_sub{font-size:14px}.ldw_enter{min-height:82px;border-radius:24px}.ldw_micro{font-size:10px;line-height:1.45}.ldw_card{border-radius:22px}.ldw_card_body,.ldw_card_head{padding:14px}.ldw_footer a{font-size:10px;padding:9px 11px}}
`;
...
`;
  ...
  function render() {
    let app = document.getElementById("ldw_app") || document.getElementById("ld_app");
    if (!app) {
    ...
    Array.from(document.body.children).forEach((el) => {
      if (el.id !== "ldw_app" && el.tagName !== "SCRIPT" && el.tagName !== "STYLE" && el.tagName !== "LINK") {
