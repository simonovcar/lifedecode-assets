...
function spawnPipe() {
  ...
  const centerY = (H - GROUND_H) * 0.52;

  // manj random premika, bolj centered gap
  const drift = (Math.random() - 0.5) * 90;
  const gapCenter = centerY + drift;

  let gapTop = gapCenter - pipeGap / 2;
  let gapBottom = gapCenter + pipeGap / 2;

  // clamp da ni retardirano dolg zgornji ali spodnji steber
  if (gapTop < safeTop) {
    gapTop = safeTop;
    gapBottom = gapTop + pipeGap;
  }

  if (gapBottom > H - GROUND_H - safeBottom) {
    gapBottom = H - GROUND_H - safeBottom;
    gapTop = gapBottom - pipeGap;
  }
