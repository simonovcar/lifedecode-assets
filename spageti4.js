      ...
      <div class="ld_cards">
        <div class="ld_card">
          ...
          <div class="mid">
            ...
            <div class="ld_hint" id="ld_checkin_hint">Tap a mood → then “New Check-In” to log today.</div>
          </div>
      ...
      <div class="ld_coach_body">
        ...
        <div class="ld_coach_footer">
          ...
          <div class="ld_coach_metaItem">
            ...
            <div class="v" id="ld_coach_mode_home">Recovery</div>
          </div>
        </div>
      </div>
    </div>
...
if (coachModeHome) {
  coachModeHome.textContent = STATE.onboard?.focus || "Recovery";
}
...
async function generateCheckinComment({ mood, action1, action2, note }) {
  ...
  const payload = {
    ...
    focus: STATE.onboard?.focus || "Recovery",
    ...
    premium: isPremium(),
    prompt: `
You are LifeDecode Recovery AI.

The user has just completed a daily check-in.

Write a SHORT, sharp, motivating response.
It should feel like a smart AI recovery coach reacting to the check-in.
...
function buildCheckinFallbackComment(mood, action1, action2) {
  const moodMap = {
    ...
    STABLE: "Good. Stable days are where real recovery gets built.",
    ...
    <div class="ld_coach_output">
      ...
      <div class="ld_coach_body">
        ...
        <div class="ld_coach_footer">
          ...
          <div class="ld_coach_metaItem">
            <div class="k">Mode</div>
            <div class="v">${STATE.onboard?.focus || "Recovery"}</div>
          </div>
        </div>
      </div>
    </div>
        ...
        <div class="ld_panel" style="border-radius:22px; margin-bottom:12px;">
          ...
          <div class="bd">
            <div class="ld_out">${
              plans.length
                ? plans.map(x =>
                    `• ${fmtDate(x.ts)} — Tone: ${x.tone || "Recovery"}\n${x.text || "—"}`
                  ).join("\n\n--------------------\n\n")
...
function showCheckinAiComment(text) {
  openSheet(
    ...
    <div class="ld_panel" style="border-radius:22px;">
      <div class="hd">
        <div>
          <div class="kicker">RECOVERY AI</div>
  ...
  if (!STATE.onboard.done) openOnboarding(false);
    ...
    function bindDelegatedClicks() {
      document.addEventListener("click", async (e) => {
        const navBtn = e.target.closest("#ld_nav button[data-open], #ld_mnav .ld_mbtn[data-open], #ld_center [data-open], #ld_right [data-open], #ld_sheet [data-open]");
        if (navBtn) {
          e.preventDefault();
...
Write a brutally practical, high-value recovery / self-control / anti-relapse action plan for the NEXT 12 HOURS.
...
Checking mood, trigger pressure, urgency, and recovery direction...`;
}
