...
async function ldHardSaveSharedXpToDB(reason = "xp-hard-save", xpDelta = 0) {
  ...
  // IMPORTANT:
  // If page/world both earn XP while one side is stale, max(local, remote) loses the newer action.
  // With a positive delta we treat this call as an XP transaction: newest DB XP + gained XP.
  ...
  if (result.error) {
    LD_SHARED_XP_LAST_SAVE_RESULT = { ok: false, reason, xp: finalXp, error: result.error.message || String(result.error), at: new Date().toISOString() };
    throw result.error;
  }

  LD_SHARED_XP_LAST_SAVE_RESULT = { ok: true, reason, xp: finalXp, at: new Date().toISOString() };
  LD_SHARED_XP_LAST_REMOTE_AT = Date.now();
  LD_SHARED_XP_LOCAL_DIRTY_UNTIL = Date.now() + 2500;
  console.log("[LD] XP hard-saved to Supabase:", LD_SHARED_XP_LAST_SAVE_RESULT);
  return true;
}
