...
(function () {
  ...
  function injectCss() {
    ...
    st.textContent = `
      ...
      .ldpa-google{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;margin-top:10px;}
  ...
  function setMode(next) {
    ...
    if (mode === "signup") {
      ...
      panel.innerHTML = `
        <div class="ldpa-notice"><strong>Create your account</strong><br>Save progress, sync your LifeDecode account, and use the same login across the page and World.</div>
        <input class="ldpa-field" id="ldpa-name" autocomplete="name" placeholder="Your name">
        ...
        <button class="ldpa-btn ldpa-google" id="ldpa-google" type="button">Continue with Google</button>
        <div class="ldpa-small">After signup, check your email and confirm your account. This page redirects back to lifedecode.app, not the World.</div>
      `;
      setTimeout(() => document.getElementById("ldpa-name")?.focus(), 60);
      return;
    }
    ...
    panel.innerHTML = `
      <div class="ldpa-notice"><strong>Welcome back</strong><br>Login to continue with your saved progress and account features.</div>
      <input class="ldpa-field" id="ldpa-email" type="email" autocomplete="email" placeholder="Email">
  ...
  async function submitSignup() {
    if (busy) return;
    const name = String(document.getElementById("ldpa-name")?.value || "").trim();
    const email = String(document.getElementById("ldpa-email")?.value || "").trim().toLowerCase();
    const password = String(document.getElementById("ldpa-password")?.value || "");
    if (!name || !email || !password) return showStatus("Fill all signup fields first.");
    ...
    try {
      ...
      if (error) throw error;
      if (!data?.session) showStatus("Account created. Check your email to confirm it.", "ok");
      else showStatus("Account created and logged in.", "ok");
      updateLoginButton();
    } catch (err) {
