...
async function ldHardSaveSharedXpToDB(reason = "xp-hard-save") {
  ...
  const mergedWorld = {
    ...
    ...localWorld,
    xp: finalXp,
    level: xpMeta.level || remoteWorld.level || localWorld.level || undefined,
    xpSystem: {
...
window.LD_DEBUG_ADD_XP = async function (amount = 1) {
  ...
  renderStats();
  await ldHardSaveSharedXpToDB("manual-add-xp");
  return window.LD_DEBUG_STATE();
};
/* === END LD SHARED XP SYNC PATCH === */
  ...
  if (clean.world && typeof clean.world === "object") {
    clean.world = {
      ...
      xp: clean.xp,
      level: clean.xpMeta.level,
      xpSystem: { ...((clean.world && clean.world.xpSystem) || {}), xp: clean.xp, total: clean.xp, level: clean.xpMeta.level }
    };
  }
...
function getSavedTriggers() {
  return Array.isArray(STATE.triggers) ? STATE.triggers : [];
    ...
    async function tryGrantXP(key, amount, cooldownMs, successText) {
      ...
      renderStats();
      await saveState();
      await ldHardSaveSharedXpToDB(key); // direct DB save so polling/world cannot drag XP back
    ...
    async function addXP(amount) {
      ...
      renderStats();
      await saveState();
      await ldHardSaveSharedXpToDB("addXP");
    }
  ...
  openSheet(
    ...
    async function maybeGrantGameReward(score) {
      ...
      if (granted > 0) {
        ...
        LD_SHARED_XP_LOCAL_DIRTY_UNTIL = Date.now() + 15000;
        await saveState();
        await ldHardSaveSharedXpToDB("game-reward");
        renderStats();
