(function () {
  ...
  const CFG = {
    VERSION: "lifedecode-app-v6-auth-premium-24h-checkin-xp",
...
`;
  ...
  async function requestDailyCheckinAI(text) {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(typeof raw === "string" ? raw : ("AI request failed: " + res.status));
      }

      const reply = extractAIReply(raw);
      return {
        ok: true,
      ...
      return {
        ok: false,
        reply: buildLocalCheckinAdvice(text) + "\n\nNote: Make webhook did not return properly, so this fallback kept the check-in alive instead of leaving the user staring into the void like a cursed loading screen."
      };
    }
  }
    ...
    for (const [fn, args] of rpcAttempts) {
      try {
        ...
        if (!error) {
          ...
          payload.lastAwardAt = new Date().toISOString();
